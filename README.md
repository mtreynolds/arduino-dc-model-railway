...
From the information we could find online, we found that most model railway trains operate at 12V. Each train has one or more 12V DC motors in them which rotate and therefore rotate the wheels and move the train. One rail of the track is connected to 12V and the other side is connected to ground (0V) and the current flows through the rails, through the wheels, through the motor and back the other side. The direction of travel is controlled by changing the polarity of the signal e.g. +12V and -12V.
...
By the time we had finished adding components we had a jumbled mess of wires. It became incredibly important to be organised and to track each connection. We had 5 station sensors and 5 track sections all of which had to be wired to the correct pin on the arduino and the code that was running the arduino had to reference them correctly.
...
Particularly attentive readers may have spotted in the final prototype board a second set of relays labeled 'Points relay' and they may have noticed in the track diagram multiple sections of points. Writing our code in the Arduino it is possible to trigger a set of points to switch automatically and thus increasingly the complexity of the layout even further.
